...
const demoProducts = [
  {
    id: "123",
    name: "Автоматический выключатель Schneider Easy9 16A",
    sku: "EZ9F34216",
    price: 2500,
    available: true,
    image: "⚡",
  ...
  {
    id: "124",
    name: "Автоматический выключатель IEK BA47-29 16A",
    sku: "MVA20-1-016-C",
    price: 890,
    available: true,
    image: "🔌",
  ...
  {
    id: "125",
    name: "Дифференциальный автомат ABB 16A",
    sku: "DS201 C16",
    price: 12400,
    available: false,
    image: "🛡️",
...
const money = (value) =>
  value === null ? "Цена уточняется" : `${value.toLocaleString("ru-RU")} ₸`;
...
function App() {
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      text: "Здравствуйте! Я помогу подобрать электротехнические товары. Что вы ищете?",
    },
  ]);
  ...
  const handleSearch = async (event) => {
    ...
    try {
      const found = apiUrl
        ? await searchCatalog(apiUrl, query)
  ...
  const refreshCart = async () => {
    ...
    try {
      ...
      if (request === cartRequest.current)
        setCartError(
          "Не удалось обновить корзину. Отображаемые данные могут быть устаревшими.",
        );
    } finally {
  ...
  useEffect(() => {
    refreshCart();
    const updated = (event) => {
      if (event.detail?.base === apiUrl) refreshCart();
    };
    window.addEventListener("ekt-cart-updated", updated);
    return () => {
      window.removeEventListener("ekt-cart-updated", updated);
      cartRequest.current++;
    };
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setStorageError(
        "Браузер не разрешил сохранить корзину. Она доступна до обновления страницы.",
      );
    }
  }, [cart]);
  useEffect(() => {
  ...
  const sendMessage = async (event, retry = false) => {
    ...
    try {
      let data;
      if (apiUrl) {
        data = await requestChat(message, aiHistory);
        setAiHistory(data.history);
      } else {
        await new Promise((resolve) => setTimeout(resolve, 650));
        data = {
          message:
            "Демонстрация: показываю тестовые товары. Реальный подбор появится после подключения сервера.",
      ...
      setError(failure.message || "Не удалось получить ответ.");
    } finally {
      busy.current = false;
      setLoading(false);
    }
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <div className="brand">
          ...
          <div>
            ...
            <small>Умный помощник по товарам</small>
          </div>
        </div>
        <button className="cart-trigger" onClick={() => setCartOpen(true)}>
          <ShoppingCart size={19} /> Корзина <b>{cartCount}</b>
        </button>
      </header>
      <div className="service-strip">
        Электротехника для ваших идей.{" "}
        <a href="#assistant">Подберите с AI-консультантом ↗</a>
      </div>
      <section className="store-intro">
        <div className="hero-heading">
          ...
          <div className="hero-help">
            ...
            <a href="#assistant">Спросить AI-консультанта ↗</a>
          </div>
        </div>
        <div className="categories" aria-label="Категории товаров">
          {[
            "Автоматы",
            "Кабели",
            "Розетки",
            ...
            <button onClick={() => chooseCategory(name)} key={name}>
              <div className={"category-art art-" + index}>
                <div />
              </div>
              <span>{name}</span>
            </button>
        ...
        <h2 className="section-title">
          Хороший выбор. <span>Для каждой вашей задачи.</span>
        </h2>
        <div className="feature-grid">
          <button
            className="feature feature-dark"
            onClick={() => chooseCategory("автоматические выключатели")}
          >
            <span className="feature-kicker">НАДЁЖНОСТЬ В ДЕТАЛЯХ</span>
            <h3>
              Под контролем.
              <br />
              Каждая линия.
            </h3>
            <p>Автоматические выключатели</p>
            <div className="breaker-art" aria-hidden="true">
              <div className="breaker-top">
                EKT <small>16A</small>
              </div>
              <div className="breaker-switch" />
          ...
          <button
            className="feature feature-blue"
            onClick={() => chooseCategory("освещение")}
          >
            <span className="feature-kicker">БОЛЬШЕ СВЕТА</span>
            <h3>
              Идеи, которые
              <br />
              освещают дом.
            </h3>
            <p>Освещение для вашего пространства</p>
          ...
          <button
            className="feature feature-white"
            onClick={() => chooseCategory("розетки и выключатели")}
          >
            <span className="feature-kicker">ПРОСТО. КРАСИВО.</span>
            <h3>
              Всё на своём
              <br />
              месте.
            ...
            <div className="socket-art" aria-hidden="true">
              <div>
                <i />
                <i />
              </div>
      ...
      <div className="assistant-title">
        <h2 className="section-title">
          Ваш консультант. <span>Сложный выбор — простой разговор.</span>
        </h2>
      </div>
      <main className="workspace" id="assistant">
        <section className="chat-panel">
          ...
          >
            {messages.map((message, index) => (
              <div className={`message-row ${message.role}`} key={index}>
                <div className="message-avatar">
                  {message.role === "assistant" ? <Bot size={16} /> : "Вы"}
                </div>
            ...
            {products.length > 0 && (
              <div className="result-note">Результаты поиска</div>
            )}
          </div>
          {error && (
            <div className="feedback" role="alert">
              ...
              <button
                disabled={loading}
                onClick={(event) => sendMessage(event, true)}
              >
                Повторить
              </button>
            </div>
          ...
          <form className="composer" onSubmit={sendMessage}>
            <input
              ...
              placeholder="Например: Найди автомат на 16 А"
            />
            <button
          ...
          <div className="suggestions">
            <button onClick={() => setInput("Найди автомат на 16 А")}>
              Автомат на 16 А
            </button>
            <button onClick={() => setInput("Подбери аналог дешевле")}>
              Аналог дешевле
            </button>
          </div>
        </section>
        <section className="catalog-panel">
          ...
          <form className="catalog-search" onSubmit={handleSearch}>
            ...
            <div>
              <input
                ...
                placeholder="Например: 16А или EZ9F34216"
              />
              <button disabled={!query.trim() || searching || loading}>
                {searching ? "Ищем…" : "Найти"}
              </button>
            </div>
            <small>
              {apiUrl
                ? "Поиск в каталоге через сервер команды"
                : "Демопоиск по трём тестовым товарам"}
            </small>
          </form>
          {searchError && (
            <p className="feedback" role="alert">
              {searchError} Нажмите «Найти», чтобы повторить.
            </p>
          )}
          ...
          {products.length ? (
              ...
              {visibleProducts.map((product) => (
                <article className="product-card" key={product.id}>
                  ...
                  <div className="product-info">
                    <span
                      className={
                        product.available ? "stock" : "stock unavailable"
                      }
                    >
                    ...
                    <h3>{product.name}</h3>
                    {product.sku && <p className="sku">Арт. {product.sku}</p>}
            ...
            <div className="empty-catalog">
              ...
              <p>
                {searched
                  ? "Попробуйте другой артикул или более короткое название."
                  : "Введите название или артикул в поиске выше."}
              </p>
            </div>
          )}
        </section>
      ...
      >
        <aside className="cart-drawer">
          ...
          {cartLoading && <p role="status">Обновляем корзину…</p>}
          {cartError && (
            <p role="alert">
              {cartError} <button onClick={refreshCart}>Повторить</button>
            </p>
          )}
          {apiUrl && (
            <p className="checkout-note">
              Общая серверная демо-корзина. Изменение количества и удаление
              здесь пока недоступны.
            </p>
          )}
          {cart.length ? (
            <div>
              ...
              <p className="checkout-note">
                {apiUrl
                  ? "Содержимое получено из backend. Оформление заказа пока недоступно."
                  : "Корзина сохранена в этом браузере."}
              </p>
              <button className="checkout" onClick={() => setCartOpen(false)}>
                Продолжить подбор
              </button>
            </div>
          ) : (
            <div className="empty-cart">
              ...
              <span>Добавьте товар из результатов поиска</span>
            </div>
          )}
        </aside>
      </dialog>
      ...
      >
        ...
        <strong>
          {pendingProduct ? money(pendingProduct.price) : ""} · 1 шт.
        </strong>
        <div className="confirm-actions">
          <button autoFocus onClick={() => setPendingProduct(null)}>
            Отмена
          </button>
          <button
            disabled={addingToCart}
            onClick={() => pendingProduct && addToCart(pendingProduct)}
          >
            {addingToCart ? "Добавляем…" : "Подтвердить добавление"}
          </button>
        </div>
      </dialog>
    </div>
